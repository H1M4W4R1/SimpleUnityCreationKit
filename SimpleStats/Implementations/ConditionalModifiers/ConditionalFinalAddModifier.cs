...
{
    /// <summary>
    ///     Final add modifier that only applies when <see cref="ShouldApply"/> returns true.
    ...
    /// </summary>
    [Serializable]
    public abstract class ConditionalFinalAddModifier<TStatisticType> : IStatModifier<TStatisticType>, IConditionalModifier
        where TStatisticType : StatisticBase
    {
        ...
        public ConditionalFinalAddModifier(float baseValue)
        {
            BaseValue = baseValue;
        }
