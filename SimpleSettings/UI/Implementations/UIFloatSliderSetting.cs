...
namespace Systems.SimpleSettings.UI.Implementations
{
    ...
    ///     <see cref="float"/>.
    ///     Implements <see cref="IWithLocalContext{ISetting}"/> so other components
    ///     in the same hierarchy can access the bound setting via the context system.
    /// </summary>
    [RequireComponent(typeof(UnityEngine.UI.Slider))]
    public sealed class UIFloatSliderSetting : UISliderBase, IWithLocalContext<ISetting>
    {
        ...
        {
            _setting = _binding.Resolve<Setting<float>>();
            if (_setting == null) return;

            // Apply ISliderSetting metadata if available.
